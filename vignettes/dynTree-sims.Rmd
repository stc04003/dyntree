...
```{R tree1, cache = TRUE}
fit1 <- dynTree(Surv(Time, delta) ~ ., id = id, data = subset(dat1, select = -c(a, b)), 
                ensemble = FALSE, trans = TRUE, control = list(numFold = 0))
fit1
```
A pruned tree can be constructed as follow:
```{R tree2, cache = TRUE}
fit2 <- dynTree(Surv(Time, delta) ~ ., id = id, data = subset(dat1, select = -c(a, b)), 
                ensemble = FALSE, trans = TRUE)
fit2
```
Lastly, turning `ensemble = TRUE` applies the ensemble method:
```{R tree3, cache = TRUE}
fit3 <- dynTree(Surv(Time, delta) ~ ., id = id, data = subset(dat1, select = -c(a, b)), 
...
The error square is 
```{R, eval = FALSE}
t0 <- seq(0, quantile(datTest$Time - 1, .9), length.out = 50)
